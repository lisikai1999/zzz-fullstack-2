<template>
  <div class="max-w-5xl mx-auto px-4 py-8">
    <div class="flex flex-col md:flex-row gap-8">
      <div class="flex-1">
        <div class="mb-6">
          <h1 class="text-3xl font-bold text-gray-900 mb-2">最新文章</h1>
          <p class="text-gray-500">探索创意与技术的交汇点</p>
        </div>
        ...
        <div class="relative mb-6">
          <input
            ...
            type="text"
            placeholder="搜索文章..."
            class="w-full px-4 py-3 pl-10 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none"
          />
            ...
            <router-link :to="`/article/${article.slug}`" class="block p-6">
              <div class="flex items-center gap-2 mb-3">
                <span
                  ...
                  :key="tag.id"
                  class="text-xs bg-primary-50 text-primary-600 px-2 py-1 rounded-full"
                >
                  {{ tag.name }}
                </span>
              </div>
              <h2 class="text-xl font-semibold text-gray-900 group-hover:text-primary-600 transition mb-2">
                {{ article.title }}
              </h2>
      ...
      <aside class="w-full md:w-72 space-y-6">
        <div class="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
          <h3 class="font-semibold text-gray-900 mb-3">站点统计</h3>
        ...
        <div class="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
          ...
          <div class="flex flex-wrap gap-2">
            <router-link
              ...
              :key="tag.id"
              :to="`/tag/${tag.slug}`"
              class="text-sm bg-gray-100 text-gray-600 px-3 py-1 rounded-full hover:bg-primary-50 hover:text-primary-600 transition"
            >
